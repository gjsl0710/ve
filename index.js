...
setTimeout(function(){
    client.on('ready', () => {
        ...
        console.log("─────────────────────────────────────────────────────────로그──────────────────────────────────────────────────────────");
            ...
            if(ss == 0){     
            client.user.setActivity("테스트", { url: 'https://www.twitch.tv/yosamonov1', type: 'STREAMING' });
            ss = ss + 1;
            }else{
                const xizy = client.users.cache.array().find(m => m.id === "703885094011797554")
                const xizy_tag = "! MOON" + "#" + 2020
...
client.on('message', message =>{
    ...
    if(message.channel.id !== "757231148933054467" && message.channel.id !== "710878363857977404") return;
    }
